...
 
$URL="https://intranet.tstech.com/sites/JSOX/"
  
     $site = Get-SPSite $URL
    
     #Write the Header to "Tab Separated Text File"
        "Site Name`t  URL `t Group Name `t User Account `t User Name `t E-Mail" | out-file "C:\PSExports\JSOX_Users.csv"
         
     #Iterate through all Webs
      foreach ($web in $site.AllWebs) 
      {
        #Write the Header to "Tab Separated Text File"
        "$($web.title) `t $($web.URL) `t  `t  `t `t " | out-file "C:\PSExports\JSOX_Users.csv" -append
         #Get all Groups and Iterate through    
         foreach ($group in $Web.groups) 
         {
                "`t  `t $($Group.Name) `t   `t `t " | out-file "C:\PSExports\JSOX_Users.csv" -append
